...
interface HeatmapProps {
  className?: string;
  data?: number[]; // Array of intensity levels 0-4
}

export function Heatmap({ className, data }: HeatmapProps) {
  // Generate dummy data if none provided (14 weeks * 7 days = 98 cells)
  const heatmapData = data || Array.from({ length: 98 }).map(() => {
    // Bias towards 0 for a more realistic looking empty-ish heatmap
